import { BiGlobe } from "react-icons/bi"
import { RiUserFill, RiUserAddFill } from "react-icons/ri"
import React, { useContext, useEffect, useState } from "react"
import LoginModal from "../features/auth/LoginModal"
import LanguageModal from "../features/language/LanguageModal"
import RegisterModal from "../features/auth/RegisterModal"
import ApproveLoginWithCodeModal from "features/auth/ApproveLoginWithCodeModal"
import { Link } from "react-router-dom";
import { CommonElementsVisibilityContext } from "features/common/CommonElementsVisibilityContext"

function Header() {
  const [showLoginModal, setShowLoginModal] = useState(false);
  const [showRegisterModal, setShowRegisterModal] = useState(false);
  const [showLanguageModal, setShowLanguageModal] = useState(false);
  const { setCommonElementsVisibility, commonElementsVisibility } = useContext(CommonElementsVisibilityContext);

  useEffect(()=> {
    setCommonElementsVisibility({ ...commonElementsVisibility, header: true, footer: true });
  },[])

  return (
    <div className={`bg-brand-color sticky h-11  top-0 z-50 w-full ${!commonElementsVisibility.header && "hidden"}`}>
      <div className="container mx-auto xl:w-[1232px]  h-11 flex items-center justify-between">
        <Link to={`/`} className=" bg-primary-brand-color mt-2 pb-4 px-5 pt-3 rounded-t-md ">
          <img className="w-[35px] h-[16px]"
            data-testid="main-image"
            src="data:image/svg+xml;base64,"
          ></img>
        </Link>

        <nav className="flex gap-x-8 text-sm font-semibold">
          <button className="flex items-center gap-x-2 text-white transition  text-opacity-80 hover:text-opacity-100 " onClick={() => setCommonElementsVisibility({ ...commonElementsVisibility, languageModal: true })}>
            <BiGlobe size={20} />
            Türkçe TR
          </button>
          <button
            className="flex items-center gap-x-2 text-white transition  text-opacity-80 hover:text-opacity-100 "
            onClick={() => setCommonElementsVisibility({ ...commonElementsVisibility, loginModal: true })}
          >
            <RiUserFill size={20} />
            Giriş Yap
          </button>
          <button className="flex items-center gap-x-2 text-white transition  text-opacity-80 hover:text-opacity-100 " onClick={() => setCommonElementsVisibility({ ...commonElementsVisibility,  registerModal: true })}>
            <RiUserAddFill size={20} />
            Kayıt Ol
          </button>
        </nav>
      </div>
      {commonElementsVisibility.languageModal && <LanguageModal setShowLanguageModal={setShowLanguageModal} showLanguageModal={showLanguageModal}  />}
      {commonElementsVisibility.loginModal && <LoginModal setShowLoginModal={setShowLoginModal} showLoginModal={showLoginModal} setShowRegisterModal={setShowRegisterModal}/>}
      {commonElementsVisibility.registerModal && <RegisterModal setShowRegisterModal={setShowRegisterModal} showRegisterModal={showRegisterModal} />}
      {commonElementsVisibility.approveLoginWithCodeModal && <ApproveLoginWithCodeModal />}

    </div>
  )
}

export default Header;
